...
/// A dot that marks a row and breathes, for a list meant to feel alive at all
/// times. Used today at the head of an AI action row.
///
/// It deliberately avoids the thing that makes ambient motion tiring, which is
/// not the movement itself but a **countable beat**. Three dots pulsing on the
/// same 2-second period lock the eye within seconds; three dots on mutually
/// prime periods, started out of phase, are never read as a rhythm at all. They
/// are felt, not noticed.
///
/// The cadence comes from `BreathCadence`, derived from the row's own seed, so a
...
struct BreathingDot: View {
    ...
    /// - Parameters:
    ///   - seed: stable identity of the row — any value that lives as long as
    ///     the row does, typically its model's id.
    ///   - isPaused: holds the breath still. An expanded row sets this: once it
    ///     is open it already has the reader's attention and its own sweep of
    ///     light, so a second ambient motion in the same card competes for
...
///
/// Both values are folded out of the row's seed so they are stable for the life
/// of the row and spread evenly across the list, without any view needing to
