...
struct SpecularSweep: ViewModifier {
    ...
    /// A single bright sliver, symmetrical, with nothing dark in it.
    ///
    /// An earlier version trailed the highlight with a faint shade to give the
    /// band an edge. It backfired: on this backdrop the ink read far more
    /// strongly than the white, so the whole sweep looked like a shadow passing
    /// in front of the card rather than light falling on it — a smudge, not a
    /// reflection. Light only, now.
    ///
    /// The stops sit between 0.26 and 0.74 rather than spanning the gradient end
    /// to end, so the band keeps a defined edge instead of fading across the
    /// whole card, which is what made the very first version invisible.
...
extension View {
    /// Adds the agent's slow sweep of light to an open card.
    /// - Parameters:
