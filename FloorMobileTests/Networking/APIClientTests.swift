...
// Serialized because MockURLProtocol.handler is shared static state.
...
struct APIClientTests {
    ...
    func bearerHeaderIsSent() async throws {
        ...
        try await Self.makeClient().send(Endpoint(method: .get, path: "clients"))
    ...
    func getEncodesQuery() async throws {
        ...
        try await Self.makeClient().send(Endpoint(
            method: .get,
            path: "clients",
    ...
    func decodesDates() async throws {
        ...
        let stamps: [Stamped] = try await Self.makeClient().send(Endpoint(method: .get, path: "stamps"))
    ...
    func malformedResponseIsDecodingError() async {
        ...
        await #expect(throws: AppError.self) {
            let _: Client = try await Self.makeClient().send(Endpoint(method: .get, path: "clients/1"))
        }
    }
    ...
    func serverErrorIsMapped() async {
        ...
        await #expect(throws: AppError.self) {
            try await Self.makeClient().send(Endpoint(method: .delete, path: "clients/1"))
        }
    }
    ...
    func retriesOnceOn401() async throws {
        ...
        let client: Client = try await Self.makeClient().send(Endpoint(method: .get, path: "clients/1"))

        #expect(client.name == "Marie")
    ...
    private nonisolated static func makeClient() -> APIClient {
        APIClient(
